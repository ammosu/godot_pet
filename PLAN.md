...
└─ prompts/
   ├─ persona.md                   # 角色個性與對話方式（可被使用者編輯）
   ├─ functions.md                 # 所有造型共用的功能協定（不由個性編輯器修改）
...
- 社群 `pet.json` 是不受信任輸入。`spritesheetPath` 只能指向造型包內的相對路徑，拒絕絕對路徑、
  `res://`／`user://` 等 scheme 與 `..`；名稱、描述與版本也要先檢查型別及長度，壞 metadata
  退回安全預設，不能讓一張本來可用的圖在強型別指派時把 loader 弄垮
...
- **拒絕之後不能只是把 `[look]` 吃掉**：共用功能規則寫著「需要看螢幕就只輸出 `[look]`」，小模型會聽規則而不聽附加的但書，於是它再要一次、標記又被吃掉，寵物就整句話都沒了。要用 `build_system_prompt(false)` 把 `## 看螢幕` 整段從 `functions.md` 拿掉那一輪
...
- `functions.md` 要明確開例外。原本寫「你看不到螢幕」，模型就算拿到圖也會回「我又沒長眼睛」
...
- **Retina 主螢幕接非 Retina 投影機時，Godot 的視窗座標來回會差 1px**：實測要求 `(2345, 338)`、讀回 `(2344, 338)`。原本用完全相等判斷 WM 是否禁止視窗掛出螢幕，這 1px 會讓 macOS 被誤認成 GNOME；之後 `_clamp_window()` 只看目前螢幕的 work area，視窗就永遠跨不過去。WM 探測現在容許每軸 1px 的座標量化誤差，真正被 mutter 推回數百 px 的情況仍會命中
...
- `functions.md` 要跟看螢幕一樣開例外，不然模型會對著明明貼在訊息裡的內容說「我看不到檔案」
...
### 2026-08-03 —— 預設寵物的近距離游標互動

芽尾的 16 方位素材原本無條件追著游標：游標落在角色上半身時，相對角色中心必然朝上，於是
經常切到背面幀，看起來像刻意背對使用者。現在命中角色本體時維持正面／側面待機，游標在角色
外才使用完整 16 方位；在本體上累積足夠移動距離且冷卻結束後，會輪流揮手、跳一下。這只套用
在內建芽尾，社群 pack 的既有反應不變。圖集的跳躍列會把角色逐格畫小，所以近距離的「跳一下」
不用那列：維持原尺寸待機幀，只改垂直位置，避免起跳前看起來突然縮水。
...
### 2026-08-13 —— 經典小遊戲「貪吃小蛇」與「踩地雷」（已完成）

新增兩個不是既有玩法換皮的經典遊戲。貪吃小蛇考持續路線與風險管理，踩地雷則完全沒有時間壓力，
考的是從數字推理安全格。兩者都沿用 `MiniGame` 的三段難度、分難度紀錄與同一個遊戲視窗。

- 貪吃小蛇讓目前造型的寵物直接當蛇頭，身體才是程式繪製的色塊；方向鍵／WASD 與點擊都能轉彎，
  每一幀之間只排一個方向，不能從身體裡原地回頭。撞牆或撞到自己扣一格，三次結束
- 踩地雷支援滑鼠左鍵翻格、右鍵插旗，也支援方向鍵、空白鍵與 F；重按已翻開的數字可以在旗數正確
  時一次展開周圍。第一格與相鄰八格保證沒有雷，避免開局純靠運氣
- 踩地雷的寵物會在棋盤下跟著游標移動並回應安全格或爆炸；不把角色塞進格子裡，否則 12×12
  棋盤上的造型會小到只剩雜訊
- `MiniGame` 多一個右鍵鉤子，沒有覆寫它的舊遊戲仍然忽略右鍵；色票集中在 `PetStyle`
- 新增 headless 測試，固定蛇身碰撞的尾格規則、踩地雷鄰格／首格安全圈，以及兩個永久紀錄 id

### 2026-08-13 —— 經典小遊戲「小蜜蜂」（已完成）

寵物在底部當玩家機體，蜂群在上方編隊左右移動；碰到邊界後整隊下降，個別蜜蜂會離隊沿曲線俯衝，
也會朝寵物所在位置發射花粉彈。這補的是「瞄準、射擊、同時閃躲」，不是把接東西或敲磚塊倒過來。

- 方向鍵／A D 與滑鼠都能移動，空白鍵／Enter／點擊發射；同時最多四發，必須等畫面上的光點離開
  或命中，不能只靠連點把整欄塞滿
- 三段難度會改變編隊大小、橫移速度、射擊頻率、俯衝頻率、同時俯衝數與花粉彈速度
- 敵人以原創的圓形、翅膀與條紋程式繪製，不借用任何街機圖像；第一排用柿子色區分並值較高分
- 俯衝路徑在中點逼近玩家、終點重新接上仍在移動的編隊，不會瞬移；headless 測試固定起點、中點、
  回隊終點與圓形碰撞邊界
- 清空後進下一波，移動與攻擊逐波加速；被花粉彈、俯衝敵人或落到底部的編隊碰到都扣一格，三次結束
- 玩家子彈與俯衝蜜蜂用「這一幀的相對移動線段」判定碰撞，而不是只比兩個幀尾座標；兩者迎面移動時
  相對速度很高，舊的離散判定會讓視覺上穿過蜜蜂的子彈偶爾漏掉

### 2026-08-14 —— 小遊戲「怪潮倖存」（已完成）

新增一個以移動與成長選擇為核心的短局生存模式。場上的角色仍是目前安裝的桌寵造型；怪物使用
Pixel-Boy 與 AAA 的 Ninja Adventure CC0 圖集，經驗結晶與攻擊光點則維持程式繪製。

- 方向鍵／WASD 四向移動，也可點擊目的地；攻擊自動瞄準最近怪物，玩家把注意力留給走位
- 怪物從四邊持續生成，速度、生命與密度會隨時間提高；精英與後段大型怪提供較多分數和經驗
- 擊倒怪物掉落經驗結晶，靠近後會被吸取；升級時場面暫停並提供三個隨機能力，支援滑鼠或 1／2／3
- 升級能力涵蓋傷害、攻速、多重射擊、移速、吸取範圍、穿透、範圍武器與回復，設有上限避免失控
- 每局固定 75 秒，存活每秒與擊倒怪物都計分；碰撞扣一格，三格用完提前結束，三段難度各自記錄
- 自動攻擊用最近距離測試，快速光點則用線段與圓的交會判定；headless 測試固定兩者的邊界行為
- 普通怪、精英怪與大型怪分別使用豬、綠武士與藍忍者的 16px 動畫圖集；只帶入實際使用的三張圖，
  來源、版本與 CC0 條款記在 `assets/third_party/ninja_adventure/LICENSE.md`
- 接觸傷害不用桌寵動畫的整張外接矩形：耳朵、尾巴與透明角落會讓看似可走的怪物縫隙仍然受傷。
  玩家與三種怪各用比可見身體略小的圓形，射擊命中仍保留較寬鬆的顯示半徑
- 場地是無界世界座標，不是 660px 視窗裡的一個房間。桌寵固定在正中央，移動時網格、怪物、掉落物
  與飛行中的光點都依相機位置投影；怪物永遠從目前視野四周生成，走再遠也不會撞到隱形牆
- 武器不再只有自動追蹤光點。升級選單保證提供「月輪靈氣」解鎖：它以 128px 半徑週期攻擊周圍
  所有怪物，一級單次 1.15 略高於光點的 1.0；較長的 1.85 秒間隔讓單一目標 DPS 仍低於遠距武器，
  後續共鳴會小幅增加傷害、範圍與頻率
- 怪物也會把武器實際掉在無界地圖上，必須走近拾取，經驗磁場不會代撿。第一把有 12 隻保底，
  精英與大型怪另有較高掉落率；首次取得解鎖三發扇形飛鏢，重複拾取最多升到五級、七發，單枚傷害
  低於基礎光點。移動時朝走位方向射擊，停下時才改瞄準最近怪物，與既有自動追蹤光點形成不同玩法；
  武器離開畫面時仍有邊緣箭頭指向，不會因無界地圖而再也找不到
- 精英與大型怪另會掉落金棕色寶箱，連續擊倒三隻高階怪仍未掉落時保底。走近開箱後直接隨機強化
  一項已持有能力，不再跳選單中斷走位；已取得的扇形飛鏢也在強化池內。寶箱留在世界座標中，
  離開畫面後以金色邊緣箭頭標示方向
...
ephemeral，不寫進檔案。也就是說：問題進了聊天 → 共用功能規則白紙黑字寫著
...
- **工作區清單是每次請求現組的**（`LLMService._work_block()`），不能寫進 `functions.md`。
  資料夾是使用者開著程式時加減的，開機時載入一次的 prompt 隔一秒就過期。沒有工作區時這段
  是空的，共用規則那句否定就原封不動 —— 它本來只在有工作區之後才變成謊話。
...
### 2026-07-31 —— 本機語音 qwen3-tts（已完成）

寵物現在可以用**跑在這台機器上的神經網路語音**講話，而且那個聲音可以是你自己的 ——
`錄一段話` 錄下來的 WAV，在「我做的東西」按一下「當我的聲音」就成立。全程不連網、不花錢，
跟系統語音一樣是關掉 LLM 也能用的東西。

**整個功能是「假設它不在」寫的。** 它要三樣這個 repo 扛不起的東西：編好的 shared library、
2.1 GB 的模型、還有一個 python3。絕大多數機器上這三樣都沒有，所以什麼都不打包、什麼都不
臆測，全新 clone 的行為跟這功能不存在時**一模一樣**。

架構上的三個選擇，都是量出來的：

- **開一個 helper process，不做 GDExtension。** 把 library 連進 Godot 就等於要每個平台出一份
  binary，而且它崩潰是崩在引擎自己的行程裡 —— 這個 library 真的會：**兩個 synthesize 呼叫重疊
  會直接 SIGABRT**，不是回傳錯誤。一隻桌寵的「附加語音」不該有能力把角色本身弄掛。
- **常駐，不是每句話跑一次 CLI。** 載模型要 754 ms，而暖機後講一句八個字只要 413 ms。
  每句話都付載入成本等於把每一句的等待變成三倍。
- **回應寫進普通檔案用 byte offset 追，請求走 stdin。** 跟 `WorkService` 一模一樣的形狀，
  因為 `FileAccess` 讀 pipe 會**阻塞**。而且 helper 的 stderr 一定要自己導走 —— 每合成一句就
  無條件寫 ~22 行到 stderr，**關不掉**（`print_timing` 預設 true，而 `Qwen3TtsParams` 根本沒有
  這個欄位），塞爆沒人讀的 pipe 會讓它卡在句子中間。

效能（RTX 4090）：載入 754 ms 一次，之後每句 383–1278 ms，RTF 0.24–0.49。上游說純 CPU 大約
1.7，也就是**比講話還慢**。所以 `_watch_speed()` 拿真實 RTF 平均三句，超過 1.0 就說一次然後
繼續 —— 它發的是 `warned` 不是 `broke`：會卡的機器仍然講得出話，而不問一聲就把使用者選的
聲音換掉，才是更嚇人的那件事。

幾個查起來會很久的坑：

- **helper 不可能變孤兒。** 連 `SIGKILL` 掉 Godot（`_exit_tree()` 救不了的那種）它都會結束，
  因為 stdin 就是那條 pipe，reader thread 讀到 EOF 就退出。`shutdown()` 裡的 `OS.kill` 是禮貌，
  不是保證。
- **閒置時卸掉模型，但不結束行程。** 卸掉會還回 **2.6 GB 顯示記憶體**給你正在用的機器。
  直接結束行程更簡單，而且刻意不做：會來會走的 helper 就是一條會在腳下斷掉的 pipe。
  （寫進死掉的 pipe **不會**弄死 Godot —— 量過，40 次寫入只回 error 13 —— 但寫進去的東西
  沒人收，寵物會等一個不會來的回覆。）
- **崩潰只原諒一次**（`MAX_RESTARTS`），跟 WorkService 重試過期 session 同一個形狀。
- **`GGML_NO_BACKTRACE=1`。** 崩潰時 ggml 會 fork **gdb** 並把輸出印到 stdout。
- **`load_models()` 會優先載 `q8_0` 而不是 `f16`**，header 跟整合文件都沒寫。所以只放量化權重的
  資料夾是合法的，只認 f16 的檢查會把它擋掉。

**設定裡寫下的路徑就是答案，不是第一個猜測。** 指定的路徑不存在時要報那件事，不能 fall through
去繼續搜尋 —— 這是真的 bug，而且在這台機器上**完全看不出來**：搜尋會找到真的 library，一切看起來
都正常。它是被一個「想模擬沒有引擎的機器」但其實沒模擬到的測試翻出來的。模型的位置則是從
library 推出來的（`<library>/../models`），所以在上游的目錄結構下，找到一個就等於找到另一個。

**參考音是靜音時，引擎會給你一個聲音，不是一個錯誤。** 拿一段 13 秒的空房間錄音（峰值 0.028）
去抽 speaker embedding，它回了 1024 個格式完全正確的數字，然後寵物就用一個沒人選過的嗓音講話。
library 這邊問不到這件事，所以 helper 自己量參考音的峰值，低於 0.05 就拒絕（真人講話量到 0.26）。
遇到自己讀不懂的 WAV 時它**放行**，因為這個檢查絕不能變成它要防的那個失敗的成因。

順帶量到的：clone 有沒有生效不用相信，用基頻比就好 —— 同一段文字，預設嗓音中位數 F0 **143 Hz**，
套上 embedding 後 **246 Hz**。

**聲音是一個具名的清單，不是一個欄位。** `user://qwen3_tts/voices/<名字>.emb`，
要幾個有幾個；「說話」選單只回報目前角色，需要更換時才開選擇視窗列出完整清單，避免新增角色
一路把日常選單撐長。名字就是檔名 ——
所以改名字用改檔名、刪聲音用刪檔案，`管理聲音…` 開資料夾而不是開第六個視窗。
克隆時會**先問名字再克隆**，因為這端唯一能自己想出來的名字是錄音檔名
「錄音 2026-07-31 012304」，那是時間戳不是聲音。

`res://voices/*.emb` 首次啟動會複製到 `user://`，跟 `daemon.py` 同一套做法，
而且只在不存在時複製 —— 使用者用同名重錄過的聲音是他的。內建兩個：`anna` 與 `yu`。
資料夾的 README 寫著每個要放進來的聲音該過的那一關：embedding 是從某個真人的聲音
推導出來的，而這個 repo 是公開的，所以沿用專案對 pet pack 已經畫好的那條線。

同一個 session、同一句話量三個：預設 **177.8 Hz**、anna **292.7 Hz**、yu **153.8 Hz**。
**要比就要在同一次執行裡比** —— 預設 temperature 0.9 讓合成是隨機的，同一個嗓音在
兩天量到 115.9 Hz 跟 177.8 Hz，這個擺盪比某些聲音之間的差距還大。之前那句「克隆 B
只跟預設差 4.3 Hz」就是踩到這個，用不同次執行的數字去比。

換一台機器要準備什麼，寫在 CLAUDE.md 的「What another machine actually needs」。
...
### 2026-08-09 —— 預設與逐造型個性（已完成）

人設不再只有 repo 裡一份 `prompts/persona.md`。右鍵「造型」現在可編輯兩層：所有角色共用的預設
個性，以及目前造型的專屬個性；沒有專屬值時不是複製一份，而是持續繼承預設，所以改一次
預設就會同步影響所有仍在繼承的角色。切換造型會同步切換 `LLMService` 的有效 persona，下一次
對話立刻生效，不必重開程式。

使用者內容放在 `user://config.cfg` 的 `[prompts]` / `[pet_prompts]`，不改 repo，也不寫回社群 pack。
刻意不從社群 `pet.json` 自動載入 prompt：造型是從外部來源安裝的素材，不能因為換一張圖就讓它
在沒有明確操作的情況下取得 system prompt 的控制權。編輯器因此只保存使用者親手套用的本機覆寫。

編輯器把「使用自訂」做成明確開關：關掉就顯示唯讀的繼承內容，重新勾選會還原尚未套用的草稿；
空白自訂不能套用。

第一版把個性和協定一起放進編輯器，這是錯的抽象：一般人想改的是個性、語氣與常見對話，不是
