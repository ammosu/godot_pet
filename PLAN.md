...
### 2026-08-11 —— 麥克風與系統聲音一起錄（已完成）

錄音來源現在是兩個獨立開關，**預設兩個都開**；「行為 → 錄音來源」可只留下麥克風或系統聲音，
但不允許兩個都關。來源在開始時固定，錄音中不讓設定改變，泡泡也會一直寫出本次實際使用的來源。

Godot 的 `AudioStreamMicrophone` 只拿輸入裝置，不能直接拿 macOS 的系統輸出。macOS 端因此用一支
小型 Swift helper 經 ScreenCaptureKit 擷取系統音訊，Godot 繼續用原有靜音 bus 錄麥克風；停止後
兩條 44.1 kHz、16-bit PCM 以 0.75 倍相加並做飽和保護，存成同一個 WAV。只選一邊時不經過混音，
直接保留原始音量與資料。helper 是 arm64／x86_64 universal binary，打包進 PCK，啟動時抽到固定的
`user://helpers` 路徑執行；原始碼與重建腳本都留在 repo。

系統聲音目前只在 macOS 13+ 開放，第一次使用由 macOS 顯示「螢幕與系統音訊錄製」權限。錄音仍
完全留在本機；ScreenCaptureKit 只掛 audio output，不取畫面 frame。若雙來源中的系統端失敗，
麥克風內容仍會保存，但完成訊息會明說缺了哪一邊，不能把不完整錄音假裝成成功的雙軌錄音。

實測 helper 錄到 4.12 秒、44.1 kHz／16-bit／stereo WAV，系統 TTS 峰值 0.782104；完整 headless
套件 10 個場景通過，macOS universal app 匯出與 deep codesign 驗證也通過。
