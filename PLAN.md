...
### 2026-08-09 —— 預設與逐造型 prompt（已完成）

人設不再只有 repo 裡一份 `prompts/persona.md`。右鍵「造型」現在可編輯兩層：所有角色共用的預設
prompt，以及目前造型的專屬 prompt；沒有專屬值時不是複製一份，而是持續繼承預設，所以改一次
...
在沒有明確操作的情況下取得 system prompt 的控制權，尤其 prompt 還包含 `[look]` 與 `[work]`
這些會觸發額外流程的規則。編輯器因此只保存使用者親手套用的本機覆寫。

編輯器把「使用自訂」做成明確開關：關掉就顯示唯讀的繼承內容，重新勾選會還原尚未套用的草稿；
空白自訂不能套用。提示也直接說明情緒標記、看螢幕與做事章節被刪掉會讓對應功能失效，因為這份
prompt 目前同時是個性與協定，假裝兩者能任意分開只會製造一個看不見的故障。
