...
`Config.get_secret()` 的查找順序是：憑證庫 → 環境變數 → 執行檔／專案旁的 `.env` →
`config.cfg`。**憑證庫排第一**：你在選單裡親手貼進去的，要贏過系統裡那個可能是幾年前
別的工具留下的環境變數。（原本是環境變數優先，結果在 Windows 上一個使用者層級的
`OPENAI_API_KEY` 會被每個行程繼承，把新貼的 key 永遠蓋掉 —— 存進去了、也存對了，
但每次請求都送舊的那把，畫面上完全看不出來。）
