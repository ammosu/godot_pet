...
- **語音輸入** —— 右鍵「用聲音跟我說話」開始收音，按停止後把暫存 WAV 送到 OpenAI
  ...
  不會留在「我做的東西」
- **會議錄音** —— 「錄一段話」預設同時錄麥克風與系統聲音，會議中別人從喇叭播放的聲音也會
  進同一個 WAV。可在「行為 → 錄音來源」只留下其中一邊；系統聲音擷取目前需要 macOS 13 以上，
  第一次使用時 macOS 會要求「螢幕與系統音訊錄製」權限。錄音只存本機，不會送到任何服務
...
- 語音輸入第一次使用會先說明：只有按下停止後，那一小段暫存 WAV 才會送到 OpenAI 轉成文字；
  音檔隨即刪除，不會放進 outbox。獨立的「錄一段話」可錄麥克風和 macOS 系統聲音，但仍然只
  存本機，兩條路不共用
